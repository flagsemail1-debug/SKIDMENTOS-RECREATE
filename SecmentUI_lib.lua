...
function SECMENT:CreateWindow(WindowSettings)
	...
	if canSaveConfig then
		local folderPath = "SecmentUI/Configs/" .. WindowSettings.ConfigFolder
	...
	-- subtle gradient accent on the window border itself (replaces old sweep line)
	Create("UIGradient", {
		Rotation = 90,
		Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0.00, Palette.Accent),
			ColorSequenceKeypoint.new(0.5, SECMENT.MLGradient[2]),
			ColorSequenceKeypoint.new(1.00, Palette.Accent),
		},
	...
	Sidebar.MouseEnter:Connect(function()
		...
		tween(AvatarFrame, {Position = UDim2.new(0, 0, 0, 10)}, 0.2, Enum.EasingStyle.Quart)
	...
	Sidebar.MouseLeave:Connect(function()
		...
		tween(AvatarFrame, {Position = UDim2.new(0, 7, 0, 10)}, 0.2, Enum.EasingStyle.Quart)
		for _, nav in pairs(NavButtons) do
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:BuildConfigSection()
			local Page = self.Page or Page
			local Section = Tab.CreateSection(self, "Configuration")

			if not canSaveConfig then
				Section:CreateLabel({Text = "Your executor does not support config saving.", Style = 3})
				return Section
			end
			...
			local NameInput = Section:CreateInput({
				Name = "Config Name",
				PlaceholderText = "my config",
				CurrentValue = "",
				Enter = false,
				Callback = function(text) nameValue = text end,
			})

			local ListSection = Section

			local function refreshList(container)
				for _, child in ipairs(container:GetChildren()) do
					if child:IsA("Frame") and child.Name == "ConfigRow" then
						child:Destroy()
					end
				end
				...
				for _, path in ipairs(files) do
					...
					if fname then
						local Row = Create("Frame", {
							...
							BackgroundColor3 = Palette.CardHi,
						}, {corner(8), stroke(Palette.Border, 1), padding(12, 0, 10, 0)})
						...
						local DeleteBtn = Create("TextButton", {
							...
							BackgroundColor3 = Color3.fromRGB(46, 22, 24),
							AutoButtonColor = false,
							Font = Enum.Font.Gotham,
							Text = "Delete",
							TextSize = 10.5,
							TextColor3 = Color3.fromRGB(220, 130, 120),
						}, {corner(6), stroke(Color3.fromRGB(90, 40, 40), 1)})
						DeleteBtn.Parent = Row

						LoadBtn.MouseButton1Click:Connect(function()
							local ok2, data = pcall(function()
								return HttpService:JSONDecode(readfile(folderPath .. "/" .. fname .. ".json"))
							end)
							if not ok2 then
								warn("[Secment UI] Failed to load config '" .. fname .. "': " .. tostring(data))
								return
							end
							for flag, value in pairs(data) do
								local element = SECMENT.Options[flag]
						...
						DeleteBtn.MouseButton1Click:Connect(function()
							pcall(delfile, folderPath .. "/" .. fname .. ".json")
			...
			Section:CreateButton({
				...
				Description = "Saves every flagged element's current value.",
				Callback = function()
					if nameValue == "" then
						warn("[Secment UI] Enter a config name before saving.")
						return
					end
					local data = {}
					for flag, element in pairs(SECMENT.Options) do
					...
					if ok then
						pcall(writefile, folderPath .. "/" .. nameValue .. ".json", encoded)
						refreshList(ListContainer)
					else
						warn("[Secment UI] Failed to encode config: " .. tostring(encoded))
					end
				end,
			})
		...
		function Tab:BuildThemeSection()
			local Page = self.Page or Page
			local Section = Tab.CreateSection(self, "Theme")

			local presetRow = Create("Frame", {
				Size = UDim2.new(1, 0, 0, 40),
				BackgroundColor3 = Palette.Card,
			}, {corner(10), stroke(Palette.BorderHi, 1), padding(14, 0, 14, 0)})
			presetRow.Parent = Page
			...
			swatchHolder.Parent = presetRow

			local presets = {
				{name = "Red", color = Color3.fromRGB(214, 39, 60), dim = Color3.fromRGB(122, 22, 34)},
				{name = "Teal", color = SECMENT.MLGradient[2], dim = Color3.fromRGB(33, 90, 90)},
				{name = "Pink", color = SECMENT.MLGradient[3], dim = Color3.fromRGB(110, 53, 73)},
				{name = "Blue", color = SECMENT.MLGradient[1], dim = Color3.fromRGB(45, 70, 95)},
			}

			local function applyAccent(color, dim)
				Palette.Accent = color
				Palette.AccentDim = dim
				for _, nav in pairs(NavButtons) do
					if Window._ActivePageId == nil then end
				end
				-- re-tint currently visible accent-dependent chrome
				MainStroke.Color = Palette.Border
				for id, nav in pairs(NavButtons) do
					local isActivePage = Pages[id] and Pages[id].Visible
					if isActivePage then
						nav.Icon.ImageColor3 = Palette.Accent
						nav.Indicator.BackgroundColor3 = Palette.Accent
					end
				end
			end
			...
			for _, preset in ipairs(presets) do
				local sw = Create("TextButton", {
					...
					AutoButtonColor = false,
					Text = "",
				}, {corner(6), stroke(Color3.fromRGB(255,255,255), 1, 0.75)})
				sw.Parent = swatchHolder
				sw.MouseButton1Click:Connect(function()
					applyAccent(preset.color, preset.dim)
				end)
			end

			Section:CreateLabel({
				Text = "Changes apply immediately when you pick a color above.",
				Style = 1,
			})
	...
	-- ===================== Window:Notification =====================
	function Window:Notification(NotifSettings)
		...
		local Card = Create("Frame", {
			...
			BackgroundColor3 = Palette.Card,
			ClipsDescendants = true,
			LayoutOrder = math.floor(os.clock() * 1000) % 2147483647,
		}, {corner(11), stroke(Palette.BorderHi, 1), padding(14, 12, 14, 12)})
		...
		local IconHolder = Create("Frame", {
			...
			BackgroundColor3 = Palette.AccentDim,
		}, {corner(7)})
		...
		Create("ImageLabel", {
			...
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundTransparency = 1,
			Image = GetIcon(NotifSettings.Icon, NotifSettings.ImageSource),
		...
		Create("TextLabel", {
			...
			BackgroundTransparency = 1,
			Font = Enum.Font.GothamSemibold,
			Text = NotifSettings.Title,
			TextSize = 12.5,
			TextColor3 = Palette.Text,
			TextXAlignment = Enum.TextXAlignment.Left,
		}).Parent = Card

		if NotifSettings.Content ~= "" then
			Create("TextLabel", {
		...
		tween(Card, {BackgroundTransparency = 0, Position = UDim2.new(0, 0, 0, 0)}, 0.25, Enum.EasingStyle.Back)
		task.delay(NotifSettings.Duration, function()
			if Card and Card.Parent then
				local t = tween(Card, {BackgroundTransparency = 1, Position = UDim2.new(0, 40, 0, 0)}, 0.2, Enum.EasingStyle.Quad)
