	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSlider(SliderSettings)
			...
			local Row = Create("Frame", {
				...
				BackgroundColor3 = Palette.Card,
			}, {corner(10), stroke(Palette.BorderHi, 1), padding(14, 8, 14, 8)})
			Row.Parent = Page
			...
			local Knob = Create("Frame", {
				...
				BackgroundColor3 = Palette.Accent,
			}, {corner(6), stroke(Color3.fromRGB(255,255,255), 1, 0.7)})
			Knob.Parent = Fill
			...
			local function setFromValue(v, fire)
				...
				tween(Fill, {Size = UDim2.new(pct, 0, 1, 0)}, dragging and 0.02 or 0.15)
			...
			local function updateFromInput(inputPos)
				...
				setFromValue(min + rel * (max - min), true)
			end
			...
			function Element:Set(new)
				if new.Value then setFromValue(new.Value, false) end
			end
			function Element:Destroy() Row:Destroy() end
			return Element
		end
