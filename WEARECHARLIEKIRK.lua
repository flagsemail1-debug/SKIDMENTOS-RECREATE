local Release = "Absolute release 2"
...
local isStudio
local website = "github.com/Nex-Softworks"
...
local MASTERLIBUI = isStudio and script.Parent:WaitForChild("MASTERLIB UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		MASTERLIB:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	MASTERLIBUI.Parent = gethui()
elseif syn and syn.protect_gui then
	syn.protect_gui(MASTERLIBUI)
	MASTERLIBUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	MASTERLIBUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == MASTERLIBUI.Name and Interface ~= MASTERLIBUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "MASTERLIB-Old"
		end
	end
elseif not isStudio then
...
MASTERLIBUI.Enabled = false
MASTERLIBUI.SmartWindow.Visible = false
MASTERLIBUI.Notifications.Template.Visible = false
MASTERLIBUI.DisplayOrder = 1000000000

local Main : Frame = MASTERLIBUI.SmartWindow
local Dragger = Main.Drag
local dragBar = MASTERLIBUI.Drag
...
local Notifications = MASTERLIBUI.Notifications
...
function MASTERLIB:Notification(data)
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function MASTERLIB:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "MASTERLIB UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
			LoadingTitle = "MASTERLIB Interface Suite",
		LoadingSubtitle = "by Nex Softworks",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "MASTERLIB Interface Suite" and Release or "MASTERLIB UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(MASTERLIBUI.MobileSupport, MASTERLIBUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "MASTERLIB/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "MASTERLIB/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".MASTERLIB") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".MASTERLIB"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".MASTERLIB", FoundKey)
						end
						MASTERLIB:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "MASTERLIB Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("MASTERLIB Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("MASTERLIB Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				if Flag then
					MASTERLIB.Options[Flag] = SliderV
				end

				MASTERLIBUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = MASTERLIB.ThemeGradient
					Slider.Main.UIStroke.color.Color = MASTERLIB.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("MASTERLIB Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				MASTERLIBUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = MASTERLIB.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = MASTERLIB.ThemeGradient
				end)

				if Flag then
					MASTERLIB.Options[Flag] = ToggleV
				end
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("MASTERLIB Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("MASTERLIB Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("MASTERLIB Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				if Flag then
					MASTERLIB.Options[Flag] = DropdownV
				end
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("MASTERLIB Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					MASTERLIB.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = MASTERLIB:SaveConfig(inputPath)
					if not success then
						MASTERLIB:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = MASTERLIB:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = MASTERLIB:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = MASTERLIB:LoadConfig(selectedConfig)
					if not success then
						MASTERLIB:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = MASTERLIB:SaveConfig(selectedConfig)
					if not success then
						MASTERLIB:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(MASTERLIB.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(MASTERLIB.Folder .. "/settings/autoload.txt")
			...
			if isfile(MASTERLIB.Folder .. "/settings/autoload.txt") then
				local name = readfile(MASTERLIB.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if MASTERLIB.Options[Flag] then
						MASTERLIB.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if MASTERLIB.Options[Flag] and data.value then
						MASTERLIB.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if MASTERLIB.Options[Flag] and data.text and type(data.text) == "string" then
						MASTERLIB.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if MASTERLIB.Options[Flag] and data.value then
						MASTERLIB.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			...
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if MASTERLIB.Options[Flag] and data.color then
						MASTERLIB.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						MASTERLIB.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						MASTERLIBUI.ThemeRemote.Value = not MASTERLIBUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						MASTERLIB.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						MASTERLIBUI.ThemeRemote.Value = not MASTERLIBUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						MASTERLIB.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						MASTERLIBUI.ThemeRemote.Value = not MASTERLIBUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				MASTERLIB.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				MASTERLIB.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function MASTERLIB:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = MASTERLIB.Folder .. "/settings/" .. Path .. ".MASTERLIB"

			local data = {
				objects = {}
			}

			for flag, option in next, MASTERLIB.Options do
		...
		function MASTERLIB:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = MASTERLIB.Folder .. "/settings/" .. Path .. ".MASTERLIB"
		...
		function MASTERLIB:LoadAutoloadConfig()
			if isfile(MASTERLIB.Folder .. "/settings/autoload.txt") then
				...
				local success, err = MASTERLIB:LoadConfig(name)
				if not success then
					return MASTERLIB:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function MASTERLIB:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(MASTERLIB.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".MASTERLIB" then
					local pos = file:find(".MASTERLIB", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			MASTERLIBUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			MASTERLIBUI.MobileSupport.Visible = false
	...
	MASTERLIBUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		MASTERLIBUI.MobileSupport.Visible = false
...
if (getgenv and not getgenv().ConfirmMASTERLIB) or (not getgenv) then
	MASTERLIB:Notification({ 
    	Title = "MASTERLIB Is Deprecated",
    	Icon = "warning",
    	ImageSource = "Material",
    	Content = "If you are not the script developer, ignore this message. \n\n The MASTERLIB Interface Library Is Deprecated And Not Recommended to Use. A New Library Is Available at nexsoftworks.xyz/starlight. If you insist on using MASTERLIB, set the getgenv().ConfirmMASTERLIB variable to true. "
	})
end

if isStudio then
	local Window = MASTERLIB:CreateWindow({
			Name = "Nex Client - MASTERLIB Hub | Blade Ball",
		Subtitle = "by Nex Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Nex Client (MASTERLIB Hub)",
		LoadingSubtitle = "Loading script for Blade Ball",
		KeySystem = true,
		KeySettings = {
		Title = "Nex Client | Key System",
		Subtitle = "Blade Ball",
		Note = "Please Enter Your Key To Use Nex Client",
			FileName = "Key",
			SaveKey = true,
			Key = {"Example Key"},
			SecondAction = {
	...
	MASTERLIB:Notification({
		Title = "Welcome to MASTERLIB",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the MASTERLIB Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nex Softworks. MASTERLIB was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. MASTERLIB Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using MASTERLIB Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "MASTERLIB's Dropdowns Has a built in Player Dropdown!",
    ...
    window = MASTERLIB:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    MASTERLIB:Notification({
        ...
        Icon = "sparkle",
        ImageSource = "Material",
        Content = "Welcome to the MASTERLIB interface, this is basically made by sussy and secment, made as a private ui lib, this is very trademarked do not steal pls !! (jk) " .. website
    })
    t1:CreateSection()
    ...
    t1:CreateParagraph({Text = "masterlib at " .. website})
